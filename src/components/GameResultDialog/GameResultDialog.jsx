...
const GameResultDialog = ({ base, ...props }) => {
  if (!props) return;
  const state = { loserScore: null, winnerScore: null };
  const {
    bracketId,
    classes,
    currentResult,
  ...
  if (!currentResult.winner || !currentResult.loser) {
    return <UnableToUpdateResult {...props} />; /* eslint consistent-return: 0 */
  }
  const handleUpdateScore = (participant, score) => {
    state[participant] = score;
  };
  const handleSave = async () => {
    if (parseInt(state.loserScore, 10) > parseInt(state.winnerScore, 10)) {
    ...
    await fetch(`${base}/game/${currentResult.game.id}`, { // TODO: If game already has a result, update the entire bracket
      ...
      onSave(data.map(d => d['Attributes'])); /* eslint dot-notation: 0 */
      onClose();
  ...
  return (
    <Dialog open={currentResult.open} classes={{ paper: classes.dialogPaper }}>
      <DialogTitle>
        Round:
        {currentResult.game && currentResult.game.round_name}
      </DialogTitle>
      <DialogContentText>
        Update Result
      </DialogContentText>
