...
class Bracket extends Component {
  ...
  renderTournament() { // Please do not try to make any sense of this function
    ...
    Object.keys(rounds).forEach((key) => {
      ...
      if (Object.keys(lastRound).length) {
        ...
        Object.keys(thisRound).forEach((gameId) => {
          ...
          const top = participants && participants[0]
            ? this.findUserById(participants[0].winner.participantId)
            : null;
          const bottom = participants && participants[1]
            ? this.findUserById(participants[1].winner.participantId)
            : null;
          ...
          lastRound[game.next_game] = lastRound[game.next_game]
            ? lastRound[game.next_game].concat([game.result])
            : [game.result];
        });
      } else {
        round.forEach((game) => {
          ...
          const top = participants && participants.length
            ? this.findUserById(participants[0])
            : null;
          const bottom = participants && participants.length
            ? this.findUserById(participants[1])
            : null;
          gameList.push(
            <BracketGame
              key={game.id}
    ...
    const tournamentWinner = (
      this.state && this.state.bracket && this.state.bracket.winner
    ) /* eslint react/destructuring-assignment: 0 */
      ? this.findUserById(this.state.bracket.winner) /* eslint react/destructuring-assignment: 0 */
  ...
  render() {
    ...
    return (
      this.state.bracket
      && this.state.bracket.id
      && this.state.rounds
      && Object.keys(this.state.rounds).length
    ) ? (
